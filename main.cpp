...
int main(int argc, char **argv)
{
    ...
    std::vector<int64_t> inputXShape = {16, 16};
    std::vector<int64_t> inputYShape = {16, 16};
    std::vector<int64_t> outputZShape = {16, 16};
    ...
    {
        inputXHostData[i] = aclFloatToFloat16(3.0);
        inputYHostData[i] = aclFloatToFloat16(2.0);
        outputZHostData[i] = aclFloatToFloat16(0.0);
        std::cout << "inputXHostData[" << i << "] = " << inputXHostData[i] << " | ";
        std::cout << "inputYHostData[" << i << "] = " << inputYHostData[i] << " | ";
        std::cout << "outputZHostData[" << i << "] = " << outputZHostData[i] << std::endl;
    }
    std::vector<void *> tensors = {inputX, inputY, out};
    ...
    ret = CreateAclTensor(outputZHostData, outputZShape, &outputZDeviceAddr, aclDataType::ACL_FLOAT16, &out);
    CHECK_RET(ret == ACL_SUCCESS, DestroyResources(tensors, deviceAddrs, stream, deviceId); return FAILED);
    ...
    ret = aclnnMaxCustomGetWorkspaceSize(inputX, inputY, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMaxcustomGetWorkspaceSize failed. ERROR: %d\n", ret);
    ...
    ret = aclnnMaxCustom(workspaceAddr, workspaceSize, executor, stream);
